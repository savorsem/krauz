...
export const generateVideo = async (
  ...
  if (generatedVideos?.length > 0) {
    ...
    if (response?.raiMediaFilteredReasons && response.raiMediaFilteredReasons.length > 0) {
        ...
        throw new Error(`Видео заблокировано фильтром безопасности: ${reason}`);
    }

    console.error('[Veo Service] Empty response:', JSON.stringify(operation, null, 2));
    throw new Error('Видео не создано. Возможно, сработали фильтры безопасности или неверная конфигурация (для Pro требуется 16:9/720p).');
  }
};